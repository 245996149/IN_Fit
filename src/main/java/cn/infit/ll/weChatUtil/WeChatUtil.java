...
import cn.inphoto.entity.WeChatUserInfo;
import cn.inphoto.util.GetInfoPro;
...
public class WeChatUtil {
	...
	static String appid = GetInfoPro.getInfo("appid");
	static String appsecret = GetInfoPro.getInfo("appsecret");
	...
	public static WeChatToken getAccessToken(String appId, String appSecret) {
		...
		System.out.println("获取公众号Token授权凭证---------->" + "拼接WeChatToken请求地址：" + requestUrl);
		// 获取网页授权凭证
		JSONObject jsonObject = HttpRequest.httpsRequest(requestUrl, "GET", null);
		System.out.println("获取公众号Token授权凭证---------->" + "接收到WeChatToken的json：" + jsonObject.toString());
		if (null != jsonObject) {
			try {
				wat = new WeChatToken();
				...
				System.out.println("获取公众号Token授权凭证---------->" + "获取WeChatToken授权凭证失败: " + errorCode + "," + errorMsg);
			}
		}
		System.out.println("获取公众号Token授权凭证---------->" + "获取到的公众号Token：" + wat.toString());
		return wat;
	}
	...
	public static WeChatToken judgeWeChatTokenOvertime(HttpServletRequest request, HttpServletResponse response)
		...
		if (token == null) {
			...
			System.out.println("判断WeChatToken是否有效中---------->" + "token为空，获取新的token");
			...
			token = WeChatUtil.getAccessToken(appid, appsecret);
			application.setAttribute("token", token);
			System.out.println("判断WeChatToken是否有效中---------->" + "token即将超时，获取新的token");
		} else {
			logger.info("判断WeChatToken是否有效中---------->" + "token可以使用，" + application.getAttribute("token").toString());
			System.out.println(
					"判断WeChatToken是否有效中---------->" + "token可以使用，" + application.getAttribute("token").toString());
		}
		System.out.println("判断WeChatToken是否有效中---------->" + "比对后获得的WeChatToken：" + token.toString());
		return token;
	}
	...
	public static WeChatUserInfo getWeChatUserInfo(String token, String openid) {
		...
		logger.info("获取公众号的用户信息中--------->" + "拼接WeChatUserInfo请求地址：" + requestUrl);
		System.out.println("获取公众号的用户信息中--------->" + "拼接WeChatUserInfo请求地址：" + requestUrl);
		JSONObject jsonObject = HttpRequest.httpsRequest(requestUrl, "GET", null);
		logger.info("获取公众号的用户信息中--------->" + "接收到UserInfo的json：" + jsonObject.toString());
		System.out.println("获取公众号的用户信息中--------->" + "接收到UserInfo的json：" + jsonObject.toString());
		if (null != jsonObject) {
			try {
				...
				System.out.println("获取公众号的用户信息中--------->" + "获取用户信息失败: " + errorCode + "," + errorMsg);
			}
		}
		logger.info("获取公众号的用户信息中--------->" + "获取到的公众号的用户信息为：" + wcUserInfo.toString());
		System.out.println("获取公众号的用户信息中--------->" + "获取到的公众号的用户信息为：" + wcUserInfo.toString());
		return wcUserInfo;
	}
}
